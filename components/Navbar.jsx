  ...
  return (
    <div className="flex justify-between items-center px-4 sm:px-2 lg:px-24 xl:px-40 py-4 sticky top-0 z-20 backdrop-blur-xl font-medium">
      ...
      <div className="flex items-center gap-2">
        ...
        <Link href="/" className="font-bold text-xl text-[#bc383b]">
          VolumeUp
        </Link>
      </div>
      ...
      <div
        className={`text-gray-700 sm:text-sm ${open ? 'max-sm:w-60 max-sm:pl-10' : 'max-sm:w-0 max-sm:overflow-hidden'
          } max-sm:fixed top-0 bottom-0 right-0 max-sm:min-h-screen max-sm:h-full max-sm:flex-col max-sm:bg-[#482928] max-sm:text-white max-sm:pt-20 flex sm:items-center gap-5 transition-all`}
      >
        {/* Mobile header */}
        <div>
          <Image
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <Link
          href="#contact-us"
          className="text-sm max-sm:hidden flex items-center gap-2 bg-primary text-white px-6 py-2 rounded-full hover:scale-105 transition-all bg-[#72120e]"
        >
          Connect
